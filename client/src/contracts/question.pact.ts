      ...
      it("should create a new question", async () => {
        await makeRequest({
          ...
          .onReceive.then((result) => {
            ...
            expect(result.data.data.user.user_id).not.toBeUndefined();
          })
          .catch();
      });
    });
        ...
        beforeAll(() => {
          return provider
            .addInteraction({
              ...
              uponReceiving:
                "a request to create a question that already exists",
              withRequest: {
                path: "/api/questions",
                method: "POST",
                body: {
          ...
          return provider
            .addInteraction({
              state:
                "ABC question exists with new category and question type ids",
              uponReceiving:
                "a request to create a question without a category",
              withRequest: {
                path: "/api/questions",
                method: "POST",
                body: {
